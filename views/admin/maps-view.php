	...
	<div class="row" style="margin-top: 20px;">
		<div class="col-md-8 col-sm-12">
			<div class="row">
				...
				<div class="col-sm-3">
					<a class="btn btn-info" href="<?php echo $admin_url ?>admin.php?page=wm_edit_website_map&tp=ver"><?php _e('Add New Vertical Map', 'websitemap'); ?></a>
				</div>
...
-moz-border-radius: 10px; border-radius: 10px; font-size:15px; margin-bottom:20px">&nbsp;<b style="color:#C00">limited time offer!</b><br  /><br  />If you want to add horizontal trees to create organizational tree or family tree please <a href="http://www.wp-buy.com/product/wp-tree-pro/" target="_blank">upgrade to the pro</a> version and save 40%, The premium version of Tree website map is completely different from the free version as there are a lot more features included<p></p></div>
            ...
            <table class="table table-bordered table-striped table-hover wm-maps-table">
				...
				<?php foreach( $website_maps['data'] as $map ): ?>
					<tr id="tr-mp_id-<?php echo $map['mp_id'] ?>">
						<td>
							<img src="<?php echo plugins_url( 'images/' . strtolower( $map['mp_type'] ) . '_16.png', WM_PLUGIN_MAIN_FILE ) ?>" border="0" width="16" height="16" />
						</td>
						<td><a href="<?php echo $admin_url ?>admin.php?page=wm_edit_website_map&mpid=<?php echo $map['mp_id'] ?>"><?php echo $map['mp_name'] ?></a></td>
						...
						<td>
							<a href="<?php echo $admin_url ?>admin.php?page=wm_edit_website_map&mpid=<?php echo $map['mp_id'] ?>">
								<img src="<?php echo plugins_url( 'images/edit_16.png', WM_PLUGIN_MAIN_FILE ) ?>" border="0" 
								title="<?php _e('Edit', 'websitemap') ?>" width="16" height="16" />
							</a>
						</td>
						<td>
							<a href="javascript: void(0);" onclick="wm_deleteMap('<?php echo $map['mp_id'] ?>');">
								<img src="<?php echo plugins_url( 'images/delete-icon_16.png', WM_PLUGIN_MAIN_FILE ) ?>" border="0" 

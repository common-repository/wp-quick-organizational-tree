...
if( empty( $mp_id ) ){
	$mp_id = wm_create_new_vertical_map();
		?>
		<div style="background: #CCEBC9 url(<?php echo  plugins_url( 'images/success2_24.png', WM_PLUGIN_MAIN_FILE ) ?>) no-repeat 10px 10px; padding: 10px 10px 10px 40px; border: solid 1px #B0DEA9; color: #508232; margin: 20px 10px;">Vertical map is created successfully</div>
	...
	<form method="post" action="" onsubmit="return false;" id="wm_vertical_tree_form">
		...
		<div class="row">
			<div class="col-sm-4">
				<div class="row">
					<div class="col-sm-4">
						<a class="btn btn-primary" style="margin-bottom: 20px;" href="<?php echo get_admin_url() ?>admin.php?page=wm_website_maps">
		...
		<div class="row">
			...
			<div class="col-sm-2">
				...
				<input type="number" class="form-control" name="mp_list_posts" value="<?php echo $map['mp_list_posts'] ?>" />
			</div>
		</div>
		...
		<div class="row">
			...
			<div class="col-sm-3">
				...
				<input type="text" class="form-control" name="mp_root_node_link" value="<?php echo $map['mp_root_node_link'] ?>" >
			</div>
		</div>
		
		
	</form>
    <div class="col-sm-12" style="color:#F00; padding:5px; font-size:14px; background:#FF9; margin-top:5px; margin-bottom:5px">
				Note : To exclude nodes and enable hyperlinks, please <a href="http://www.wp-buy.com/product/wp-tree-pro/" target="_blank">upgrade</a> to pro version
			</div>
            <p></p>
